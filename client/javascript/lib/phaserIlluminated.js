...
Phaser.Plugin.PhaserIlluminated.prototype._construct = function(){
    ...
    this._game.add.illuminated.tilemapObject = this._createOpaqueObjectsFromSolidTiles;
...
Phaser.Plugin.PhaserIlluminated.prototype._createOpaqueObjectsFromSolidTiles = function(tileMapLayer){
    if(!tileMapLayer){
        return [];
    }
    ...
    for(var r = 0; r < tileMapLayer.layer.data.length; r++){
        ...
        for(var c = 0; c < row.length; c++){
            ...
            if(tile.canCollide){
                ...
                var tempObj = this._game.add.illuminated.rectangleObject(tile.x+tile.width, tile.y+tile.height, tile.width, tile.height);
                opaqueObjects.push(tempObj);
            }
...
Phaser.Plugin.PhaserIlluminated.prototype.createOpaqueObjectsFromSolidTiles = function(tileMapLayer){
    ...
    for(var r = 0; r < tileMapLayer.layer.data.length; r++){
        ...
        for(var c = 0; c < row.length; c++){
            ...
            if(tile.canCollide){
                //opaqueObjects.push(this._createRectangleObject(tile.x+tile.width, tile.y+tile.height, tile.width, tile.height));
                var tempObj = this._createRectangleObject(tile.x+tile.width, tile.y+tile.height, tile.width, tile.height);
                opaqueObjects.push(tempObj);
            }
